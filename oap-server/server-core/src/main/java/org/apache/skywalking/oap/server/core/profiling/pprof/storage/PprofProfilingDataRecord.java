...
public class PprofProfilingDataRecord extends Record {
    ...
    /**
     * Pprof event type (cpu, heap, block, mutex, etc.)
     */
    ...
    /**
     * Serialized pprof profiling data
     */
    ...
    public static class Builder implements StorageBuilder<PprofProfilingDataRecord> {
        ...
        public PprofProfilingDataRecord storage2Entity(final Convert2Entity converter) {
            final PprofProfilingDataRecord dataRecord = new PprofProfilingDataRecord();
            dataRecord.setTimeBucket(((Number) converter.get(TIME_BUCKET)).longValue());
            dataRecord.setTaskId((String) converter.get(TASK_ID));
            dataRecord.setInstanceId((String) converter.get(INSTANCE_ID));
            dataRecord.setUploadTime(((Number) converter.get(UPLOAD_TIME)).longValue());
            dataRecord.setEventType((String) converter.get(EVENT_TYPE));
            dataRecord.setDataBinary(converter.getBytes(DATA_BINARY));
            return dataRecord;
        }
